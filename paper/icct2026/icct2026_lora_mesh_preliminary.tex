...
reception from SNR margin. Using this harness, we compare managed flooding, a
    ...
    baseline, with 839.6 s versus 971.5 s of aggregate airtime. A separate
    ...
    variants. These results support a bounded preliminary claim rather than
    universal dominance.
\end{abstract}
...
We evaluate repeated unicast traffic and mixed traffic. Repeated unicast uses a
fixed set of source-destination pairs so that route caching has a meaningful
opportunity to help. Mixed traffic contains both unicast and broadcast flows,
which is closer to many IoT mesh deployments that combine telemetry, control,
...
failures. We report means over 20 random seeds. Each seed changes node
placement, traffic timing, selected pairs, the static per-link shadowing
realization, and stochastic reception outcomes. The frozen main matrix also
uses the historical shared random stream in which forwarding jitter and
reception draws can consume the same generator. The released simulator
provides an opt-in independent channel-randomness mode for new runs; this
separates reception draws from forwarding jitter and learning exploration, but
it still does not create event-by-event common random numbers when protocols
...
flooding or generalizes to arbitrary pair churn. A fair follow-up should report
random-pair traffic separately and should pre-register a non-degenerate
direct-link PRR distribution and multi-hop route fraction.
...
The preliminary results support four observations. First, fixed flooding and
...
recovery unavailable to the fixed baselines. This is a protocol-bundle result,
not an isolated estimate of confidence ranking. Redundancy should therefore be
treated as a per-context budget rather than a static network-wide mode.

Fourth, the two experiments answer different questions: route-conflict isolates
...
must disable recovery for all protocols or equalize retry and fallback budgets.
...
traffic use eight fixed pairs, and no random-pair workload is included.
Consequently, the current data are not sufficient to claim calibrated
multi-hop deployment performance.
...
recovery controller. The frozen \texttt{calm-mesh} line uses a zero
confidence-fallback threshold, while the exploratory Smart-CALM variants also
allow timeout retries and profile changes. Their reliability cannot therefore be
attributed to confidence ranking alone. Moreover, reception, forwarding jitter,
and exploration share one stream in the frozen matrix: same seeds fix
...
Future work will calibrate a genuine multi-hop regime using a pre-registered
direct-link PRR distribution and cached-route hop distribution, add random-pair
workloads, equalize retry and fallback budgets, use the independent channel
random stream (or keyed per-event draws), model variable packet headers,
introduce time-varying links, and validate the design with hardware-in-the-loop
experiments.
...
where link-quality evidence is informative. Because fixed pairs and unequal
recovery budgets remain, these results should be read as preliminary
common-harness evidence rather than a universal ranking of LoRa mesh protocols.
...
\url{https://github.com/BH4ME/lora-mesh-routing-sim}. No human participants or
personal data were involved. The authors report no conflicts of interest and no
external funding. Author contributions cover protocol design, implementation,
experiment design, analysis, and manuscript preparation. AI-based tools
assisted with code inspection and language editing; the authors verified the
implementation, numerical results, and final manuscript.
