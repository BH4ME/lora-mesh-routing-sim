...
reception from SNR margin. Using this harness, we compare managed flooding, a
    ...
    forwarding for route-recovery cases. In the frozen 50-node mixed-traffic
    matrix, which is explicitly a contention-oriented repeated-pair case,
    MeshEcho reaches 0.735 ACK-confirmed unicast PDR versus 0.389 for the
    source-route baseline, with 839.6 s versus 971.5 s of aggregate airtime.
    The same matrix is link-quality saturated: calculated direct-link PRR is
    above 0.99 for all node pairs and almost all cached routes are one hop. A
    paired cache-lifetime audit shows that matching the source-route baseline's
    cache TTL to MeshEcho's 600 s raises its ACK PDR to 0.519, reducing the
    native mean gap by 0.138 while leaving a 0.209 ACK-PDR gap in the matched
    protocol-bundle comparison.
    A separate
    ...
    reverse controls produce no gain. An additional ten-seed, one-shot
    random-pair probe with Smart-CALM timeout retries disabled gives MeshEcho
    0.413 ACK-confirmed PDR and 97.5 s of airtime, versus 0.456 PDR and 139.5 s
    for source-route caching. Smart-CALM and its no-confidence control reach
    0.489 and 0.488 PDR, respectively, with a paired 95\% interval for their
    difference that includes zero. Recovery and route-cache audits further show
    that native protocol budgets are not identical. These results support a
    bounded preliminary reliability-airtime tradeoff claim, not universal
    dominance.
\end{abstract}
...
\begin{itemize}
    ...
    \item We quantify evaluation sensitivity to fixed-pair reuse, link-quality
    saturation, route-cache lifetime, and recovery budget instead of treating
    the frozen matrix as a neutral multi-hop benchmark.
\end{itemize}
...
generate different numbers of transmissions. Fresh ICC matrices from the
current release use the independent mode and a matched 600 s MeshCore-like
route-cache TTL by default; the frozen files remain historical artifacts for
traceability. In the main matrix,
...
\begin{table*}[t]
\centering
\caption{Frozen native repeated-unicast results over 20 seeds. MeshEcho denotes
the confidence-aware prototype. This is a fixed-pair, contention-oriented
case, not a neutral multi-hop benchmark.}
\label{tab:unicast}
\begin{tabular}{lrrrr}
\toprule
...
\begin{table*}[t]
\centering
\caption{Frozen native mixed-traffic and high-shadowing results over 20 seeds.
MeshEcho denotes the confidence-aware prototype. The fixed-pair and native
recovery limitations are discussed in the text.}
\label{tab:mixed}
...
frozen repeated-unicast result. Managed flooding reaches the highest unicast PDR in
...
same channel. Table~\ref{tab:mixed} shows that the frozen source-route baseline
...
\paragraph{Fairness judgment.}
The evaluation is fair in its shared-harness construction: topology, traffic
trace, PHY settings, static shadowing, and seed values are reused across
protocols. It is not fully neutral as a single benchmark. The frozen matrix
favors route reuse and contains almost no multi-hop link-quality conflicts,
while MeshEcho's native recovery behavior is not available to the fixed
baselines. We therefore treat the frozen tables as a declared operating-point
comparison, the matched-TTL audit as a sensitivity correction, the random-pair
probe as an external-validity check, and the route-conflict experiment as
mechanism evidence. This separation prevents the native PDR gap from being
interpreted as a pure confidence-ranking effect.
...
2.1.13). No human participants or personal data were involved. The authors
